...
model = dict(distribution=distribution, mindist=0.025, seed=True)
...
if not os.path.isfile("settings.txt"):
    ...
    settings_file.write(
        "{0:<31} {1:<20} {2:<160} {3:<7} {4:<15} {5:<7} {6:<7} {7:<7} {8:<7} {9:<7} {10:<7} {11:<7}\n".format(
            *header_items
        )
    )
else:
...
date_time = "{0}/{1}/{2} {3:02}:{4:02}:{5:02}".format(
    localtime[2], localtime[1], localtime[0], localtime[3], localtime[4], localtime[5]
)
    ...
    for i in range(n_simulations):
        ...
        if not os.path.isfile("stl.stl") or stl_overwrite:
            if stl_overwrite and os.path.isfile("stl.stl"):
                print(
                    "Overwriting stl file 'stl{0}{1}_{2}{0}stl.stl'".format(
                        os.sep, run_name, i
                    )
                )
            ...
            print(
                "WARNING: stl{0}{1}_{2}{0}stl.stl already exists, skipping this directory".format(
                    os.sep, run_name, i
                )
            )
        os.chdir("..")
...
if pre_process:
    ...
    if not os.path.isdir(basecase_dir):
        print(
            "ERROR: given OpenFOAM base case directory '{0}' does not exist".format(
                basecase_dir
            )
        )
        quit()
    ...
    if not os.path.isdir("baseCase"):
        os.mkdir("baseCase")
    subprocess.call(
        "cp -rf {0}{1}* baseCase{1}".format(basecase_dir, os.sep), shell=True
    )
    ...
    for i in range(n_simulations):
        ...
        case_stl_dir = os.path.realpath(
            "{0}{1}{2}_{3}".format(stl_dir, os.sep, run_name, i)
        )
        case_stl_file = os.path.realpath(
            "{0}{1}stl.stl".format(case_stl_dir, os.sep, i)
        )

        # Create case from baseCase if it does not exist yet, or if existing directory is not a valid OpenFOAM directory
        openfoam_dirs = [
            "{0}{1}{2}".format(case_dir, os.sep, folder)
        ...
        subprocess.call(
            [
                "cp",
                case_stl_file,
                "{0}{1}constant{1}triSurface".format(case_dir, os.sep),
            ]
        )
        ...
        preP.create_pre_processing_script(
            "{0}_{1}".format(run_name, i),
            cores_per_sim,
            tasks_per_node,
            threads_per_core,
            partition,
    ...
    # Start meshing
    waiting_cases = [
        os.path.realpath("cases{0}{1}_{2}".format(os.sep, run_name, i))
        for i in range(n_simulations)
    ]
    active_cases = []
    finished_cases = []
    ...
    if cluster:
        while len(finished_cases) < n_simulations:
            ...
            queue = dict(
                [[i[:32].strip(), [i[32:64].strip(), i[64:].strip()]] for i in queue]
            )
            for i in range(len(active_cases)):
                ...
                if not "{0}_pre".format(case_name) in queue:
                    if snappy_refinement and not os.path.isfile(
                        ...
                        case_stl_dir = os.path.realpath(
                            "{0}{1}{2}_{3}".format(stl_dir, os.sep, run_name, case_num)
                        )

                        # Get location in mesh from stl directory
                        lim_file = open(
                            "{0}{1}locationInMesh.dat".format(case_stl_dir, os.sep)
                        )
                        location_in_mesh = map(float, lim_file.readline().split())
                        lim_file.close()
        ...
        while waiting_cases:
            ...
            subprocess.call(
                [
                    ...
                    "+x",
                    "{0}{1}preprocessing{2}.sh".format(
                        case_dir, os.sep, "_0" if snappy_refinement else ""
                    ),
                ]
            )
            subprocess.call(
                ["./preprocessing{0}.sh".format("_0" if snappy_refinement else "")]
            )
            if snappy_refinement:
                ...
                subprocess.call(
                    ["chmod", "+x", "{0}{1}preprocessing_1.sh".format(case_dir, os.sep)]
                )
...
if simulations:
    ...
    for i in range(n_simulations):
        ...
        if not os.path.isdir(case_dir):
            print(
                "WARNING: case directory '{0}_{1}' does not exist, skipping this case".format(
                    run_name, i
                )
            )
            continue
        os.chdir(case_dir)
        preP.create_simulation_script(
            "{0}_{1}".format(run_name, i),
            cores_per_sim,
            tasks_per_node,
            threads_per_core,
    ...
    if cluster:
        ...
        while len(finished_cases) < n_simulations:
            ...
            while waiting_cases and len(active_cases) < n_parallel_sims:
                ...
                subprocess.call(
                    [
                        "sbatch",
                        "{0}{1}runSimulations.sh".format(new_case_dir, os.sep),
                        "&",
                    ]
                )
                os.chdir(base_dir)
            ...
            for i in end_cases:
                case = active_cases.pop(i)
                if (
                    preP.check_log("{0}{1}simpleFoam.log".format(case, os.sep))
                    or not restart_cases
                ):
                    finished_cases.append(case)
                    print("Finished cases: {0}".format(finished_cases))
                else:
                    ...
                    print(
                        "Case '{0}' failed to run properly. Current total number of fails: {1}/{2}".format(
                            case_name, n_sim_fails, n_allowed_fails
                        )
                    )
                    if n_sim_fails >= n_allowed_fails:
                        print(
                            "Maximum amount of failed cases reached ({0}), no longer restarting cases.".format(
                                n_allowed_fails
                            )
                        )
                        restart_cases = False
        ...
        while waiting_cases:
            ...
            if preP.check_log("{0}{1}simpleFoam.log".format(case_dir, os.sep)):
                ...
                if n_sim_fails >= n_allowed_fails:
                    print(
                        "Maximum amount of failed cases reached ({0}), stopping simulations".format(
                            n_allowed_fails
                        )
                    )
...
if post_process:
    ...
    for i in range(n_simulations):
        ...
        if not os.path.isdir("VTK"):
            print(
                "WARNING: no directory 'VTK' in case directory '{0}_{1}', skipping this case".format(
                    run_name, i
                )
            )
            os.chdir(base_dir)
        ...
        if not vtk_files:
            print(
                "WARNING: no .vtk files in 'VTK' directory of case '{0}_{1}', skipping this case".format(
                    run_name, i
                )
            )
        vtk_files.sort()
        ...
        if vtk_file.split("_")[-1] == "0":
            print(
                "WARNING: no .vtk files of timestep > 0 in 'VTK' directory of case '{0}_{1}', skipping this case".format(
                    run_name, i
                )
            )
            os.chdir(base_dir)
        ...
        postP.create_post_processing_script(
            os.path.realpath("."),
            "{0}_{1}".format(run_name, i),
            "{0}{1}postProcessing.py".format(this_dir, os.sep),
            "VTK{0}{1}".format(os.sep, vtk_file),
            10 ** -6,
            1000,
            margin=post_processing_margin,
        )
        ...
        while len(finished_cases) < n_simulations:
            ...
            while waiting_cases and len(active_cases) < n_parallel_sims:
                ...
                if not os.path.isfile(
                    "{0}{1}postprocessing.sh".format(new_case_dir, os.sep)
                ):
                    print(
                        "WARNING: no 'postprocessing.sh' bash script in case '{0}', skipping case".format(
    ...
    for i in range(n_simulations):
        ...
        if not os.path.isfile("out.dat"):
            print(
                "WARNING: case '{0}_{1}' has no output file 'out.dat', skipping this case".format(
                    run_name, i
                )
            )
            os.chdir(base_dir)
